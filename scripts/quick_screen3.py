...
# 修正筛选条件（turn已除100，实际是0.0247=2.47%，所以>=1表示>=100%不可能）
# 改用：turn >= 0.01（即>=1%），amount >= 3000（即>=3000万）
cond = (df['pct'] > 0) & (df['turn'] >= 0.01) & (df['main_force'] > 0) & (df['amount'] >= 3000)
df_s = df[cond].sort_values('main_force', ascending=False)
print(f"\n强势股（涨>0 + 换手>=1%% + 主力净流入>0 + 成交额>=3000万）: {len(df_s)} 只")
...
for code in top_codes:
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"{code} | {ma_arr} | 主力净流入: {mforce:.0f}万")
        ...
        print(f"  量价背离={div} 量比={vol_ratio:.2f} 5日均换手={turn5avg*100:.1f}%%")
        print(f"  60日线上方:{above_ma60} 20日位置:{pos20:.0%%} 评分:{score}")
...
print("  最终综合评分排名")
print(f"{'='*60}")
print(f"  排名  代码     收盘   涨跌   RSI6   均线   MACD   60日线  背离  评分")
for i, r in enumerate(results_sorted, 1):
    print(f"  {i:2d}.  {r['code']}  {r['close']:.2f}  {r['pct']:+.2f}%%  {r['rsi6']:5.1f}   {r['ma_arr']}  {r['macd']:+.4f}   {r['above_ma60']}   {r['div']}   {r['score']:3.0f}")