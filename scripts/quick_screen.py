...
# 筛选条件：有主力净流入 + 换手>1% + 涨幅>0
cond = (df['pct'] > 0) & (df['turn'] >= 1) & (df['main_force'] > 0)
df_s = df[cond].sort_values('main_force', ascending=False)
print(f"\n有主力净流入 + 换手>1% + 涨>0: {len(df_s)} 只")
...
for code in top_codes:
    ...
    try:
        ...
        print(f"\n{'='*55}")
        print(f"{code} {d['close'].iloc[0]} | {ma_arr} | MA5={ma5:.2f} MA10={ma10:.2f} MA20={ma20:.2f}")
